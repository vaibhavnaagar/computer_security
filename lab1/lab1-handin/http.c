...
int http_printf(char *dst, int nbytes, char *fmt, ...)
{
  if(nbytes <= 0)
    return -1;
// for now this function is used at places where atmost one extra argument is provided, so not using while loop
  va_list ap;
  va_start(ap, fmt);
  int bytes_written = vsnprintf(dst, nbytes, fmt, ap);
  va_end(ap);
...
const char *http_request_line(int fd, char *reqpath, size_t reqpath_len, char *env, size_t *env_len, size_t env_size)
{
    char buf[4096];
    char *sp1, *sp2, *qp, *envp = env;
    char *env_end = env + env_size -1;
    ...
    *sp2 = '\0';
    ...
    envp += http_printf(envp, env_size, "REQUEST_METHOD=%s", buf) + 1;
    envp += http_printf(envp, env_end - envp + 1, "SERVER_PROTOCOL=%s", sp2) + 1;

    /* parse out query string, e.g. "foo.py?user=bob" */
    if ((qp = strchr(sp1, '?')))
    {
        *qp = '\0';
        envp += http_printf(envp, env_end - envp + 1, "QUERY_STRING=%s", qp + 1) + 1;
    }

    /* decode URL escape sequences in the requested path into reqpath */
    url_decode(reqpath, sp1, reqpath_len);

    envp += http_printf(envp, env_end - envp + 1, "REQUEST_URI=%s", reqpath) + 1;
    envp += http_printf(envp, env_end - envp + 1, "SERVER_NAME=zoobar.org") + 1;
...
void env_deserialize(char *env, size_t len, size_t env_len)
{
    char *env_end = env + env_len - 1;
    char *envp = (env + len - 1);
    envp += http_printf(envp, env_end - envp + 1, "GATEWAY_INTERFACE=CGI/1.1") + 1;
    envp += http_printf(envp, env_end - envp + 1, "REDIRECT_STATUS=200") + 1;
    *envp = 0;
    len = envp - env + 1;
