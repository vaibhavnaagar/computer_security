...
pid_t launch_svc(CONF *conf, const char *name)
{
    ...
    if ((dir = NCONF_get_string(conf, name, "dir")))
    {
        ...
        if (chroot(dir) != 0)
        {
          warnx("Unable to chroot to %s", dir);
          exit(1);  // Exiting to prevent from grave security error
        }
    }
    ...
    if (NCONF_get_number_e(conf, name, "gid", &gid))
    {
        ...
        {
          warnx("Unable to change real, effective and saved gid to %d", (int)gid);
          exit(1);   // Exiting to prevent from grave security error
        }
    }
    ...
    if ((groups = NCONF_get_string(conf, name, "extra_gids")))
    {
      ...
      {
        warnx("Unable to set extra_gids");
      }
      for (i = 0; i < ngids; i++)
    ...
    if (NCONF_get_number_e(conf, name, "uid", &uid))
    {
      ...
      {
        warnx("Unable to change real, effective and saved uid to %d", (int)uid);
        exit(1);   // Exiting to prevent from grave security error
      }
    }
